...
private boolean fileStatus = true; //the file initally exists
/**
...
{  
   ...
   while (true) {
	 ...
	 if(first == false){
		...
		url = line;
   		String[] f = url.split(" ");
   		filePath = f[1];  //remove spaces
   		String[] a = filePath.split("/"); // remove forward slash /
		filePath = a[a.length-1];
		filetype = filePath.substring( (filePath.indexOf(".")+1), (filePath.length()) );//get the file type by parsing from the .
                file = new File(filePath); //make a File object using string for file path
		first = true; 
	 }//if
...
{  
	...
	catch(FileNotFoundException ex){  fileStatus = false; os.write("404 Not Found\n".getBytes());}
...
{  
if( fileStatus ){//only read file if it has been found else 404
   Date d = new Date();
   DateFormat df = DateFormat.getDateTimeInstance();
   df.setTimeZone(TimeZone.getTimeZone("GMT"));
   //if the file is an image type write it by the bytes
  if(image) {   
	int b; 
